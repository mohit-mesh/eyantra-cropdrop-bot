...
Color_t Color_Detect(void)
{
    ...
    /* Detection Logic with Increased Sensitivity (removed large negative offsets) */
    /* RED: Red must be significantly > Green and Blue */
    if ((r_n > g_n + COLOR_DIFF) && (r_n > b_n + COLOR_DIFF))
    {
        printf("RED   R=%lu G=%lu B=%lu (Diff vs G:%ld B:%ld)\n", r_n, g_n, b_n, (long)(r_n-g_n), (long)(r_n-b_n));
        RGB_LED_Red();
        return COLOR_RED;
    }
    /* GREEN: Green must be > Red and Blue */
